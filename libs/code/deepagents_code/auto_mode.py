...
def _unresolvable_write_path_reason(root: Path, raw: object) -> str | None:
    ...
    The check stays lexical. `Path.resolve` would follow a symlink or reparse
    point on the way to its answer, and refusing to traverse a model-authored
    path is the whole point of this policy pass, so the two cases `resolve`
    would have surfaced are detected directly instead.
    """
    if not isinstance(raw, str) or not raw:
        return None
    if "\x00" in raw:
        return _path_error_reason(ValueError("embedded null byte"), raw)
    try:
        candidate = Path(raw).expanduser()
        if not candidate.is_absolute():
            candidate = root / candidate
        # Lexical only: `normpath` collapses the path textually, where
        # `resolve` would ask the filesystem to follow each component.
        os.path.normpath(os.fspath(candidate))
...
class AutoModeHITLMiddleware(HumanInTheLoopMiddleware[AutoModeState, Any, Any]):
    ...
    def __init__(
        ...
        *,
        ...
        Raises:
            ValueError: If a required local path is invalid or a trusted tool has
                an unexpected name.
        """
        if (
