...
def _virtualize_local_repository_backend(
    backend: BackendProtocol,
    root: str,
) -> tuple[BackendProtocol, str, str | None]:
    """Give local repository tools stable virtual paths.

    Args:
        backend: Repository backend supplied by the caller.
        root: Trusted host repository root.

    Returns:
        Backend, backend root, and optional host root used for path translation.
    """
    from deepagents.backends.filesystem import (
        FilesystemBackend as FilesystemBackendType,
    )

    if not isinstance(backend, FilesystemBackendType) or backend.virtual_mode:
        return backend, root, None
    virtual_backend = FilesystemBackend(root_dir=root, virtual_mode=True)
    virtual_backend.max_file_size_bytes = backend.max_file_size_bytes
    return virtual_backend, "/", root
...
def _create_rubric_grader_tools(
    ...
    *,
    ...
    if (
        ...
        tool_backend, bounds_root, source_root = _virtualize_local_repository_backend(
            repository_backend,
            repository_root,
        )
        try:
    ...
    if "ls" in repository_tools:
        ...
        def ls(path: str, runtime: ToolRuntime[None, Any]) -> object:
            ...
            if error := active_bounds.preflight("ls", args):
                return error
            clamped = active_bounds.clamp_args("ls", args)
            return _bound(
                active_bounds,
                "ls",
                fs_ls_func(path=clamped["path"], runtime=runtime),
            )
...
def create_cli_agent(
    ...
    *,
    ...
    if sandbox is None and effective_cwd is None:
    ...
    if sandbox is None:
        ...
        if large_results_dir is None and use_virtual_project_root:
            large_results_dir = Path(artifacts_root) / "large_tool_results"
        if large_results_dir is not None:
