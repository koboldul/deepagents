...
def _terminate_windows_server_process(
    ...
    *,
    ...
    Request graceful Ctrl+Break shutdown first, then terminate the Job to reap
    any descendants that outlive the root. Root-process fallbacks and waits stay
    bounded so shutdown is safe to repeat.

    Args:
        process: Root server process.
        job: Kill-on-close Job Object containing the server tree, if assigned.
        graceful: Whether to request Ctrl+Break before forced cleanup.
    """
    root_exited = process.poll() is not None
    if graceful and not root_exited:
        try:
            _signal_windows_server(process)
        except ProcessLookupError:
            root_exited = True
        else:
            root_exited = _wait_for_windows_process_exit(
                process,
    ...
    if root_exited and (job is None or job_terminated):
        return
    if job_terminated:
        if _wait_for_windows_process_exit(
            process,
            timeout=_SIGKILL_TIMEOUT,
        ):
            return
        logger.warning(
    ...
    elif job is not None and root_exited:
        return
...
def _assign_windows_server_job_and_resume(
    ...
    try:
        ...
        _terminate_windows_server_process(process, job, graceful=False)
        raise
    return job
...
def _signal_windows_server(process: subprocess.Popen[Any]) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.exception(
                "Failed to terminate server process pid=%d; "
                "continuing with owned-tree cleanup",
                process.pid,
            )
        ...
        *,
        ...
        with self._state_lock:
            ...
            if sys.platform == "win32":
                ...
                creationflags = (
                    _WINDOWS_CREATE_SUSPENDED | _WINDOWS_CREATE_NEW_PROCESS_GROUP
                )
            if sys.platform == "win32":
