...
class LocalContextMiddleware(AgentMiddleware):
    ...
    run the bash detection script via `backend.execute()`. Context is collected
    on first interaction and stored for stable system-prompt injection. After
    each summarization event, changed context is appended as an internal
    conversation message so the cached prompt prefix stays byte-identical.
    """
