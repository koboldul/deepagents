...
_WINDOWS_SAFE_PREFIX_LENGTH = 16
_WINDOWS_SAFE_DIGEST_LENGTH = 40
    ...
    with _WRITE_LOCK:
        ...
        try:
            ...
            if _path_exists(path):
                prior_payload = _read_bytes(path)
                prior_revision = hashlib.sha256(
                    prior_payload.replace(b"\r\n", b"\n")
                ).hexdigest()
            ...
            _native_path(tmp_path).replace(_native_path(path))
            if os.name != "nt":
                path.chmod(_FILE_MODE)
        except OSError:
            logger.warning(
                "Failed to materialize transcript at %s",
                path,
                exc_info=True,
            )
            with suppress(OSError):
                _native_path(tmp_path).unlink()
            raise
    return revision
...
def _safe_component(identifier: str) -> str:
    ...
    if os.name == "nt":
        prefix_length = _WINDOWS_SAFE_PREFIX_LENGTH
        digest = digest[:_WINDOWS_SAFE_DIGEST_LENGTH]
    else:
        prefix_length = _SAFE_PREFIX_LENGTH
    prefix = _SAFE_PREFIX_RE.sub("-", readable).strip("-")[:prefix_length]
    return f"{prefix or 'id'}--{digest}"


def _ensure_private_directories(root: Path, target: Path) -> None:
    _native_path(root).mkdir(parents=True, exist_ok=True, mode=_DIR_MODE)
    _native_path(target).mkdir(parents=True, exist_ok=True, mode=_DIR_MODE)
    if os.name == "nt":
        return
...
def _prune_backups(path: Path, retention_revisions: int) -> None:
    ...
    with os.scandir(_native_path(path.parent)) as entries:
        backups = sorted(
            (
    ...
    for _modified, name in backups[: max(0, excess)]:
        with suppress(OSError):
            _native_path(path.parent / name).unlink()


def _read_transcript(path: Path) -> tuple[list[TranscriptRecord], bool]:
    records: list[TranscriptRecord] = []
    try:
        for line in _native_path(path).read_text(encoding="utf-8").splitlines():
            if not line.strip():
                continue
...
def _path_exists(path: Path) -> bool:
    return _native_path(path).exists()


def _is_file(path: Path) -> bool:
    return _native_path(path).is_file()


def _read_bytes(path: Path) -> bytes:
    return _native_path(path).read_bytes()
