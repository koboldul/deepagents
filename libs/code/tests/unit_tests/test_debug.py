...
class TestConfigureDebugLogging:
    ...
    @pytest.mark.skipif(sys.platform != "win32", reason="Windows ACL hardening")
    def test_debug_acl_allows_owner_cleanup(self, tmp_path) -> None:
        """Owner-only ACLs still allow deleting logs and their directory."""
        directory = tmp_path / "debug"
        log_file = directory / "thread.log"
        directory.mkdir()
        log_file.touch()

        _debug._prepare_debug_directory(directory)
        _debug._prepare_debug_file(log_file)
