...
class TestUpdateLogs:
    ...
    def test_dependency_refresh_dry_run_ignores_phantom_metadata_extras(
        ...
        assert command == (
            ...
            f"{_shell_arg('deepagents-code==1.2.3')}"
            " --prerelease allow"
        )
...
class TestInstallExtraCommand:
    ...
    def test_uv_install_extra_command_preserves_composite_extras(
        ...
        assert _install_extra_uv_tool_command(
            ...
            "uv --no-config tool install --reinstall -U "
            f"{_shell_arg(f'deepagents-code[all-providers,daytona]=={__version__}')} "
            "--prerelease allow"
        )
...
class TestPerformInstallExtra:
    ...
    async def test_lock_wraps_command_generation_and_subprocess(self) -> None:
        ...
        def command(_extra: str) -> tuple[str, ...]:
...
class TestPerformInstallPackage:
    ...
    async def test_lock_wraps_command_generation_and_subprocess(self) -> None:
        ...
        def command(_package: str) -> tuple[str, ...]:
            events.append("command")
            return ("uv", "tool", "install", "safe-command")
...
class TestRunInstallSubprocessFailureModes:
    ...
    async def test_windows_job_assignment_failure_falls_back_to_kill(self) -> None:
        """A direct terminate that does not reap the root falls back to `kill`."""
        proc = MagicMock()
        proc._transport._proc._handle = 5678
        wait_for_root_exit = AsyncMock(return_value=True)
        ...
        wait_for_root_exit.assert_awaited_once()
        proc._transport.close.assert_called_once_with()
