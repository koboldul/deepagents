...
# `ProjectContext` rejects a non-absolute path, and a leading slash is not
# absolute on Windows.
_EXECUTE_SERVER_CWD = Path(
    "C:/workspace/server" if os.name == "nt" else "/workspace/server"
)
...
def test_format_execute_description_prefers_workspace_binding(
    ...
    """
    server_context = ProjectContext(user_cwd=_EXECUTE_SERVER_CWD)
    ...
    assert str(_EXECUTE_SERVER_CWD) not in description
...
def test_format_execute_description_falls_back_on_malformed_workspace(
    ...
    assert f"Working Directory: {_EXECUTE_SERVER_CWD}" in description
    ...
    def test_cwd_sets_platform_filesystem_root_dir_without_shell(
        ...
        *,
        ...
        with (
            ...
            patch(
                "deepagents_code.agent.FilesystemBackend",
                side_effect=FilesystemBackend,
            ) as mock_filesystem,
            patch(
                "deepagents_code.agent.create_deep_agent",
                return_value=mock_agent,
